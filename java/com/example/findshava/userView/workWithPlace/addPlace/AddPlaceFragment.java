...
// нажатие назад на выборе свойств. Проверить стек фрагментов. Нужно сначала pop() (мб убрать проверка на наличие и просто replace сделать)

// нажатие при добавлении места. рисуется поверх (хз) (maybe fixed (swap add to replace))

// при двойном входе на выборе свойств добавить, то что уже выбрано. Возможно отклонить popBackStack()

// в choosePropertiesFragment не появляется то что в бд. Нужно кидать из create в choose инфу

// картинки

// selectedtext для всех, сделать отдельным классов. в infoplace тоже использовать

// раскидать файлы по папкам

// проверить сериализацию + код

// при второй попытке добавления места остается инфа от прошлого добавления

// круглишок о загрузке можно поднимать и опускать(анимация), нужно делать это с его рутом.